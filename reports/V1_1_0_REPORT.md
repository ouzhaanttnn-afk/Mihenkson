...
## TestFlight adayı — tema seçimi kaldırıldı, 19 Eylül 2026

Kullanıcının son isteğiyle v1.1.0, `ba81f8a` yedeğinden geri getirildi. Bu bölüm önceki tema notlarını geçersiz kılar: Sistem/Açık/Koyu/Build 21 seçimi, sistem görünümü dinleyicisi ve açık/koyu CSS paletleri kaldırıldı. Sabit orijinal Build 21 paleti kullanılır. Market'teki daha önce satın alınabilen dükkan dekorları aynı kalır; bunlar yeni görünüm seçicisi değildir.

Eski kayıtların tema alanları yok sayılır; diğer tercihler ve ekonomi kaydı korunur. Tema dışındaki v1.1.0 davranışı değiştirilmedi; önceki denetimde bildirilen stok gösterimi ve küçük ekran yoğunluğu bulguları bu dar kapsamlı pakette düzeltilmedi. Canlı Game Center kimlikleri hâlâ yapılandırılmamıştır.

Doğrulama: 79 dosyada 1.175 Vitest + 5 release hazırlık testi, üretim build'i ve 77 release kontrolü geçti. Native imzalı build/yükleme sonucu ayrıca GitHub Actions üzerinden doğrulanmalıdır. App Store genel yayını istenmedi; hedef yalnız TestFlight.

Omni: 1 kısa CLI denemesi, yanıt alınamadı; başarılı çıkarım 0. React incelemesi kaldırılan sistem dinleyicisinin yeniden eklenmemesi ve kayıtların mevcut normalize yoluyla korunmasına odaklandı; yeni manager eklenmedi.
