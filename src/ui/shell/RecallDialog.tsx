...
export function RecallDialog() {
  const premium = usePremium(s => s.known && s.active);
  ...
  return <div className="settingsScrim"><div className="settingsBox recallDialog" ref={dialogRef}
    ...
    <p>{premium ? t('Premium ile aynı müşteriyi reklamsız geri çağır ve son teklifini yap.') : t('Müşteri dükkândan ayrıldı. Bir reklam izleyerek aynı müşteriyi geri çağır ve son teklifini yap.')}</p>
    <button ref={initialFocusRef} className="dayCloseDialog__primary" disabled={busy || !guest.deal} onClick={() => void recall()}>
      {busy ? (premium ? t('İşlem sürüyor…') : t('Reklam yükleniyor…')) : t('Müşteriyi geri çağır')}
    </button>
